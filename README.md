...
A lightweight TypeScript widget that embeds a live-chat popup in the bottom-right corner of any website. The widget loads a Maya bot conversation from `https://maya.test/bots/<botIntegrationId>/chat` and can be configured on the fly or via a remote configuration endpoint.
...
| `baseUrl` | `string` | `https://maya.test` | Base Maya host. The chat iframe loads from `${baseUrl}/bots/<botIntegrationId>/chat`. |
